    ...
    <form-wrapper :action="action" method="POST">
        <template slot-scope="form">
            ...
            <div v-if="order.status == 'requested'">
                <p>Auftrag wurde am {{ moment(order.created_at).format('LLL') }} Uhr durch den Kunden {{ order.client.name }} angefragt.</p>
            </div>
            <div class="panel panel-default">
                <div class="panel-body">
                    <div class="row">
                        <div class="col-md-12">
                            <p>Unterauftrag von Auftrag {{ order.parent_id }}.</p>
                        </div>
            ...
            <submit-button class="pull-right btn-md btn-success" :text="type == 'edit' ? 'Speichern' : 'Anlegen'" :loading="form.loading"></submit-button>
        </template>
    </form-wrapper>
</template>
